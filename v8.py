# -*- coding: utf-8 -*-
"""
Created on Thu Dec 25 00:14:11 2025
...
class AIAnalyzer:
    """Hugging Face AI Expert with Reasoning (DeepSeek-R1)"""
    
    @staticmethod
    def analyze_with_hf(scan_results, output_widget, master, hf_token):
        """Analyze results using DeepSeek-R1 reasoning on Hugging Face"""
        try:
            # We use DeepSeek-R1 for deep security reasoning
            ...
            stream = client.chat.completions.create(
                messages=[{
                    "role": "user", 
                    "content": f"As a cybersecurity expert, perform a deep analysis of these scan results. Identify critical vulnerabilities and recommend hardening steps:\n{scan_results}"
                }],
                max_tokens=2048,
                stream=True
            )
            ...
            for chunk in stream:
                ...
                # 1. Capture 'Thinking' (Chain of Thought)
                if hasattr(delta, 'reasoning_content') and delta.reasoning_content:
                    if not in_thinking:
                        in_thinking = True
                        master.after(0, lambda: output_widget.insert(tk.END, "\n🧠 REASONING (DeepSeek Thinking):\n" + "─"*50 + "\n"))
                    
                    text = delta.reasoning_content
                    master.after(0, lambda t=text: output_widget.insert(tk.END, t))
                    master.after(0, lambda: output_widget.see(tk.END))

                # 2. Capture Final Security Report
                elif delta.content:
                    if in_thinking:
                        in_thinking = False
            ...
            master.after(0, lambda: output_widget.insert(tk.END, f"\n\n❌ API Error: {str(e)}\n"))
class CyberSecurityToolkit_GUI:
    ...
    def create_ai_tab(self):
        """Dedicated Hugging Face Tab v6.0"""
        tab = ttk.Frame(self.notebook)
        self.notebook.add(tab, text="🤗 HF Intelligence")
        
        header = tk.Frame(tab, bg=COLORS['accent_purple'], height=60)
        header.pack(fill='x')
        tk.Label(header, text="🛡️ Hugging Face Cyber Intelligence",
                font=('Arial', 16, 'bold'), bg=COLORS['accent_purple'],
                fg=COLORS['text_white']).pack(pady=15)
        
        # API Configuration
        api_frame = ttk.LabelFrame(tab, text="HF API Access", padding=15)
        api_frame.pack(fill='x', padx=10, pady=10)
        
        tk.Label(api_frame, text="HF Token:", font=('Arial', 10, 'bold'),
                bg=COLORS['bg_medium'], fg=COLORS['text_white']).grid(row=0, column=0, sticky='w', padx=5)
        
        self.hf_token_entry = tk.Entry(api_frame, width=70, font=('Arial', 9))
        self.hf_token_entry.grid(row=0, column=1, sticky='ew', padx=5)
        # Your specific token is inserted here
        self.hf_token_entry.insert(0, "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")
        
        # Input Section
        input_frame = ttk.LabelFrame(tab, text="Security Logs / Scan Results", padding=10)
        input_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        self.ai_input = scrolledtext.ScrolledText(input_frame, height=8, font=('Courier', 9),
                                                bg=COLORS['bg_dark'], fg=COLORS['text_white'])
        self.ai_input.pack(fill='both', expand=True, padx=5, pady=5)
        
        tk.Button(input_frame, text="🧠 Run Deep Reasoning Analysis", command=self.do_ai_analyze,
                 bg=COLORS['success'], fg='white', font=('Arial', 11, 'bold'),
                 height=2).pack(pady=5, fill='x')
        
        # Report Output
        output_frame = ttk.LabelFrame(tab, text="AI Security Expert Report", padding=5)
        output_frame.pack(fill='both', expand=True, padx=10, pady=5)
    ...
    def do_ai_analyze(self):
        """Execute the Hugging Face analysis"""
        scan_results = self.ai_input.get(1.0, tk.END).strip()
        hf_token = self.hf_token_entry.get().strip()
        
        if not scan_results:
            messagebox.showerror("Error", "Input scan data first.")
            return

        self.ai_output.delete(1.0, tk.END)
        self.ai_output.insert(tk.END, ">>> Initializing Hugging Face Reasoning Engine...\n")
        
        threading.Thread(target=self._ai_hf_thread, args=(hf_token, scan_results), daemon=True).start()

    def _ai_hf_thread(self, hf_token, results):
        """Threaded AI execution"""
        AIAnalyzer.analyze_with_hf(results, self.ai_output, self.master, hf_token)
# =============================================================================
# MAIN
# =============================================================================
